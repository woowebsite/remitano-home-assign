    ...
    <>
      {isOpen && (
        <div id={'toast-' + type} className='flex border px-2 items-center w-full mb-2 text-gray-500 bg-white rounded-lg shadow' role='alert'>
          {renderElement()}
          <div className='ml-3 py-3 text-md font-normal'>
            {title && (
              <>
                {title} <br />
              </>
          ...
          <button
            onClick={handleClose}
            type='button'
            className='ml-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 inline-flex h-8 w-8 dark:text-gray-600'
            aria-label='Close'
          >
